...
import type { ConnectionState, StatusRegistry } from './status';
...
export class PositionFeed {
  ...
  accept(sample: SimSample): void {
    ...
    if (!this.policy.shouldPublish(sample.isOnGround, sample.altitude)) {
      return;
    }
  ...
  private async publish(report: PositionReport): Promise<Outcome> {
    try {
      await this.publisher.publish(report);
      ...
      if (error instanceof PublisherUnauthorisedError) {
      ...
      if (error instanceof PositionRejectedError) {
