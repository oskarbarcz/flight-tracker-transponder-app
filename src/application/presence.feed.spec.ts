...
import { SessionExpiredError } from './ports/session';
...
function feed(presence: () => Promise<Presence | null>): {
  feed: PresenceFeed;
  writer: FakeWriter;
...
describe('PresenceFeed', () => {
  ...
  it('clears the activity when the API has nothing to publish', async () => {
    let current: Presence | null = payload;
    const { feed: presenceFeed, writer } = feed(() => Promise.resolve(current));
