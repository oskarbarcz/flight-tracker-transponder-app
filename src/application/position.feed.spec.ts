...
function harness(
  capacity = 10,
  policy: RatePolicy = new RatePolicy(1, 1),
  transmitting = true,
): Harness {
  ...
  feed.setTransmitting(transmitting);
...
describe('PositionFeed', () => {
  ...
  it('keeps sampled timestamps when a publish is retried', async () => {
    ...
    failWith(new PositionPublishFailedError(503));
  ...
  it('reports the ADS-B service as unauthorised on a rejected token', async () => {
    ...
    feed.setCurrentFlightCallsign('AAL4908');
    failWith(new PublisherUnauthorisedError());
  ...
  it('drops a report the service refuses rather than wedging the queue', async () => {
    ...
    failWith(new PositionRejectedError(400, 'squawk must be a string'));
    feed.accept(
      sample({ sampledAt: new Date(Date.UTC(2026, 7, 13, 12, 0, 0)) }),
    );
    await feed.drain();
  ...
  it('holds transmission back until something switches it on', async () => {
    const { feed, published, status } = harness(
      10,
      new RatePolicy(1, 1),
      false,
  ...
  it('drops to one report every thirty seconds above ten thousand feet', async () => {
    const { feed, published } = harness(10, new RatePolicy());
    feed.setCurrentFlightCallsign('LH455');
